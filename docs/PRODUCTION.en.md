...
The GitHub Actions workflow can also be started manually with `workflow_dispatch`. Pull requests and ordinary non-release pushes run validation; a push to `main` may additionally publish the optional Docker `latest` image when Docker Hub publication is enabled. A push of a `v*` tag runs the same test matrix and build gate and then, only if all prerequisite jobs succeed, runs a release job with repository-scoped `contents: write` permission. The job verifies that `vX.Y.Z` matches both `VERSION` and `[project].version` in `pyproject.toml`, downloads the artifacts generated by the build job, creates a consolidated `SHA256SUMS.txt`, and creates or updates the GitHub Release using the built-in `gh` CLI and `GITHUB_TOKEN`.
